...
public class BrickerGameManager extends GameManager {
    ...
    private int score;
    private gameobjects.GraphicLifeCounter[] lives ;
    private Counter brickCounter;
    private Counter livesCounter;
    public gameobjects.NumericLifeCounter life;
    ...
    private void generateBricks( Renderable brickImage)
    {
        ...
        for (int i = 0 ; i < NUM_BRICK_ROWS ; i++)
        {
            for (int j = 0 ; j < NUM_BRICK_COLS ; j++)
            {
                ...
                GameObject brick = new gameobjects.Brick(Vector2.ZERO, new Vector2(brickSizeX,BRICK_HEIGHT) , brickImage ,collisionStrategy , brickCounter);
    ...
    private void checkForGameEnd() {
        ...
        if (windowDimensions.y() < ballHeight)
        {
            ...
            life = new gameobjects.NumericLifeCounter(livesCounter , Vector2.ZERO , new Vector2(30,30) , gameObjects() ) ;
    ...
    {
        gameobjects.Ball ball = new gameobjects.Ball(Vector2.ZERO, new Vector2(BALL_RADIUS,BALL_RADIUS) , ballImage , collisionSound);
        this.ball = ball;
        modifyBallSpeed();
    ...
    {
        GameObject userPaddle = new gameobjects.Paddle(Vector2.ZERO, new Vector2(PADDLE_WIDTH,PADDLE_HEIGHT) , paddleImage , inputListener,windowDimensions , BORDER_WIDTH);
    ...
    private void generateScore(Renderable lifeImage)
    {
        ...
        lives = new gameobjects.GraphicLifeCounter[score];
        for (int i = 0 ; i < score ; i++)
        {
            gameobjects.GraphicLifeCounter life = new gameobjects.GraphicLifeCounter(Vector2.ZERO , new Vector2(LIFE_RADIUS,LIFE_RADIUS) , livesCounter, lifeImage , gameObjects() ,i);
